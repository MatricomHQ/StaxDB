...
struct InternalNode {
    ...
    InternalNode(uint32_t f_len) : fragment_len(f_len), _padding(0) {
        for(int i=0; i<16; ++i) children[i].store(0, std::memory_order_release);
    }

    char* get_fragment_data() { return reinterpret_cast<char*>(this) + sizeof(InternalNode); }
    const char* get_fragment_data() const { return reinterpret_cast<const char*>(this) + sizeof(InternalNode); }
    std::string_view get_fragment() const { return std::string_view(get_fragment_data(), fragment_len); }
};
