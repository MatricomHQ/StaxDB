...
void StaxCursor<Tree>::move_next() {
    ...
    if (track_aabb_ && !stack_.empty()) restore_aabb(stack_.back().aabb_log_size);
    ...
    while (!stack_.empty()) {
        ...
        for (int nibble = last_frame.nibble_in_parent + 1; nibble < 16; ++nibble) {
            ...
            if (child_ptr != 0) {
                ...
                if (track_aabb_) refine_box_for_nibble(*current_aabb_, key_nibble_offset, nibble, tree_->get_dimensionality(), this);
                stack_.push_back({last_frame.node_ptr, nibble, aabb_change_log_size_, key_nibble_offset + 1});

                uint64_t current_ptr = child_ptr;

                while (!StaxTree16::is_leaf(current_ptr)) {
                    key_nibble_offset = stack_.back().key_nibble_offset;
                    InternalNode* node = tree_->get_allocator().template get_ptr<InternalNode>(StaxTree16::get_offset(current_ptr));
                    bool found_child = false;
                    for (int i = 0; i < 16; ++i) {
                        uint64_t next_ptr = node->children[i].load(std::memory_order_acquire);
                        if (next_ptr != 0) {
                            if (track_aabb_) refine_box_for_nibble(*current_aabb_, key_nibble_offset, i, tree_->get_dimensionality(), this);
                            stack_.push_back({current_ptr, i, aabb_change_log_size_, key_nibble_offset + 1});
                            current_ptr = next_ptr;
                            found_child = true;
                            break;
