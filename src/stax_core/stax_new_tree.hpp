...
inline std::vector<void*> StaxTree16::query_knn(const uint64_t* query_point, int k, QueryStats& stats) const {
    ...
    bool forward_ok = (forward_cursor->seek_first(), forward_cursor->is_valid());
    bool backward_ok = (backward_cursor->seek_last(), backward_cursor->is_valid());

    bool forward_pruned = false;
    bool backward_pruned = false;

    std::vector<uint64_t> coords(D_); // Hoist allocation out of the loop

    auto process_cursor = [&](auto& cursor, bool& is_ok, bool& is_pruned, bool is_forward) {
        stats.leaves_visited++;
        const AABB* box = cursor->get_current_aabb();

        if (best_results.size() == static_cast<size_t>(k)) {
            long double current_max_dist_sq = best_results.top().first;
            if (SpatialKeywords::distance_to_box_sq(query_point, *box, D_) > current_max_dist_sq) {
                is_pruned = true;
                return;
            }
        }

        void* handle = cursor->get_record_handle();
        stats.records_loaded++;
        StaxRecord* record = allocator_.get_ptr<StaxRecord>(get_offset(reinterpret_cast<uint64_t>(handle)));
        SpatialKeywords::get_coords_from_apk(record->get_key(), coords.data(), D_);
        stats.records_scanned++;
        ...
        if (best_results.size() < static_cast<size_t>(k)) {
            ...
            best_results.pop();
            best_results.push({dist_sq, handle});
            accepted = true; // A better point was found
        }
        if(accepted) stats.records_accepted++;


        if (is_forward) cursor->move_next();
        else cursor->move_prev();
        is_ok = cursor->is_valid();
    };

    while (!forward_pruned || !backward_pruned) {
        stats.nodes_visited++; // Approximate node visits
        bool can_scan_forward = forward_ok && !forward_pruned;
        bool can_scan_backward = backward_ok && !backward_pruned;

        if (!can_scan_forward && !can_scan_backward) {
            break;
        }

        long double dist_fwd = can_scan_forward
            ? SpatialKeywords::distance_to_box_sq(query_point, *forward_cursor->get_current_aabb(), D_)
            : std::numeric_limits<long double>::max();

        long double dist_bwd = can_scan_backward
            ? SpatialKeywords::distance_to_box_sq(query_point, *backward_cursor->get_current_aabb(), D_)
            : std::numeric_limits<long double>::max();

        if (dist_fwd <= dist_bwd) {
            process_cursor(forward_cursor, forward_ok, forward_pruned, true);
        } else {
            process_cursor(backward_cursor, backward_ok, backward_pruned, false);
        }
    }
    ...
    std::reverse(results.begin(), results.end()); // The queue gives largest distance first.
    return results;
}
