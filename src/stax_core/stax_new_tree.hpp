...
public:
    ...
    std::vector<void*> query_box(const AABB& query_box, QueryStats* stats = nullptr) const;
    std::vector<void*> query_sphere(const uint64_t* center, long double radius, QueryStats* stats = nullptr) const;
    std::vector<void*> query_knn(const uint64_t* query_point, int k, QueryStats* stats = nullptr) const;
...
public:
    ...
    static inline int get_nibble_from_fragment(const uint8_t* fragment_bytes, uint8_t nibble_idx) {
...
inline std::vector<void*> StaxTree16::query_box(const AABB& query_box, QueryStats* stats) const {
    ...
    std::vector<uint64_t> current_coords(D_);
    ...
    if (root != 0) {
        stack.push_back({root, AABB(D_), 0});
    }
    ...
    while (!stack.empty()) {
        ...
        if (is_leaf(current.node_ptr)) {
            if (stats) stats->leaves_visited++;

            StaxRecord* record = allocator_.get_ptr<StaxRecord>(get_offset(current.node_ptr));
            if (stats) stats->records_loaded++;

            SpatialKeywords::get_coords_from_apk(record->get_key(), current_coords.data(), D_);
            if (stats) stats->records_scanned++;

            bool in_box = true;
            for (uint32_t i = 0; i < D_; ++i) {
                if (current_coords[i] < query_box.min_bounds[i] || current_coords[i] > query_box.max_bounds[i]) {
                    in_box = false;
                    break;
                }
            }
            if (in_box) {
                if (stats) stats->records_accepted++;
                results.push_back(reinterpret_cast<void*>(current.node_ptr));
            }
            continue;
        }
...
inline std::vector<void*> StaxTree16::query_sphere(const uint64_t* center, long double radius, QueryStats* stats) const {
    AABB sphere_box(D_);
    uint64_t r = static_cast<uint64_t>(radius);
    for (uint32_t i = 0; i < D_; ++i) {
        sphere_box.min_bounds[i] = (center[i] > r) ? center[i] - r : 0;
        sphere_box.max_bounds[i] = (center[i] < UINT64_MAX - r) ? center[i] + r : UINT64_MAX;
    }

    std::vector<void*> candidates = query_box(sphere_box, stats);
    std::vector<void*> results;
    results.reserve(candidates.size());
    long double radius_sq = radius * radius;
    std::vector<uint64_t> coords(D_);

    if (stats) stats->records_scanned = 0;
    ...
    if (stats) {
        stats->records_accepted = results.size();
    }
    return results;
}
...
inline void StaxTree16::insert(ThreadLocalAllocator& local_alloc, const TxnContext &ctx, std::string_view key, std::string_view value, bool is_delete) {
    while (true) {
        ...
        while (current_ptr != 0 && !is_leaf(current_ptr)) {
            ...
            std::string_view fragment = node->get_fragment();
            int d_idx = find_first_differing_nibble(key, fragment);
        ...
        if (current_ptr == 0) {
            ...
            if (existing_record->get_key() == key) {
                ...
                memcpy(new_node->get_fragment_data(), key.data(), key.length());
            ...
            InternalNode* node = allocator_.get_ptr<InternalNode>(get_offset(current_ptr));
            std::string_view rep_key = node->get_fragment();
