...
inline std::vector<void*> StaxTree16::query_knn(const uint64_t* query_point, int k, QueryStats& stats) const {
    ...
    while ((forward_cursor->is_valid() && !forward_pruned) || (backward_cursor->is_valid() && !backward_pruned)) {
        ...
        // Determine which cursor to advance based on which AABB is closer to the query point
        bool advance_forward = false;
        if (forward_cursor->is_valid() && !forward_pruned) {
            if (!backward_cursor->is_valid() || backward_pruned) {
                advance_forward = true;
            } else {
                const AABB* fwd_box = forward_cursor->get_current_aabb();
                const AABB* bwd_box = backward_cursor->get_current_aabb();
                if (SpatialKeywords::distance_to_box_sq(query_point, *fwd_box, D_) < SpatialKeywords::distance_to_box_sq(query_point, *bwd_box, D_)) {
                    advance_forward = true;
                }
            }
        }
        ...
        } else if (backward_cursor->is_valid() && !backward_pruned) {
