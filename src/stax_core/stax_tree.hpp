...
private:
    ...
    // !!! BUG FIX !!!
    // Root Cause: Functions marked with `STAX_ALWAYS_INLINE` were defined in the `.cpp` file, causing
    // a link-time error on GCC. `always_inline` functions must have their definitions available in the
    // header file where they are declared.
    // Fix: Moved the function bodies for `get_bit`, `count_leading_zeros`, and `simd_memcmp`
    // from `stax_tree.cpp` into the class definition here.
