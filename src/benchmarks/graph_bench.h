...
inline std::vector<NodeData> generate_nodes(size_t num_nodes) {
    ...
    for (size_t i = 0; i < num_nodes; ++i) {
        NodeData node;
        node.id = i + 1; // Simple sequential IDs for test data
...
inline void run_graph_benchmark() {
    ...
    for (size_t t = 0; t < BENCHMARK_NUM_THREADS; ++t) {
        ingest_threads.emplace_back([&, t]() {
            ...
            for (size_t i = t; i < properties.size(); i += BENCHMARK_NUM_THREADS) {
                ...
                if (prop.is_numeric) {
                    txn.insert_fact_numeric(prop.obj_id, prop.field_name, prop.numeric_value);
                } else {
                    txn.insert_fact_string(prop.obj_id, prop.field_name, prop.value_literal);
                }
                total_facts_ingested++;
            }
            for (size_t i = t; i < edges.size(); i += BENCHMARK_NUM_THREADS) {
                txn.insert_fact(edges[i].source_id, edges[i].rel_type_name, edges[i].target_id);
                total_facts_ingested++;
            }
            txn.commit();
        });
    ...
    {
        uint32_t user100_id = 101; 
        uint32_t user200_id = 201;
        uint32_t user500_id = 501;
        uint32_t user101_id = 102;
        uint32_t user102_id = 103;
        uint32_t user103_id = 104;
        
        GraphTransaction setup_txn(db.get(), 0);
        setup_txn.insert_fact(user100_id, "FOLLOWS", user500_id);
        setup_txn.insert_fact(user200_id, "FOLLOWS", user500_id);
        setup_txn.insert_fact(user101_id, "FOLLOWS", user102_id);
        setup_txn.insert_fact(user102_id, "FOLLOWS", user101_id);
        setup_txn.insert_fact(user100_id, "FOLLOWS", user102_id);
        setup_txn.insert_fact(user102_id, "FOLLOWS", user103_id);
        setup_txn.insert_fact(user103_id, "FOLLOWS", user100_id);
        setup_txn.commit();
        ...
        for (size_t t = 0; t < BENCHMARK_NUM_THREADS; ++t) {
            query_threads.emplace_back([&, t]() {
                ...
                for (size_t i = start_idx; i < end_idx; ++i) {
                    if(reader.get_property_for_object_string(query_node_ids[i], "email")) {
                        local_emails++;
                    }
                }
...

    {
        ...
        for (size_t t = 0; t < BENCHMARK_NUM_THREADS; ++t) {
            query_threads.emplace_back([&, t]() {
                ...
                reader.get_objects_by_property_into_roaring("city", "London", london_users_bm);
    ...
    
    {
        ...
        reader.get_objects_by_property_range_into_roaring("age", 25, 35, results_bm);
    ...
    
    {
        ...
        GraphReader reader(db.get(), read_ctx);
        roaring_bitmap_t* results_bm = roaring_bitmap_create();
        reader.get_objects_by_property_range_into_roaring("age", 0, 29, results_bm);
        size_t count = roaring_bitmap_get_cardinality(results_bm);
        roaring_bitmap_free(results_bm);
        auto end = std::chrono::high_resolution_clock::now();
        long long duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
...

    {
        ...
        GraphReader reader(db.get(), read_ctx);
        roaring_bitmap_t* results_bm = roaring_bitmap_create();
        reader.get_objects_by_property_range_into_roaring("age", 75, 99, results_bm);
        size_t count = roaring_bitmap_get_cardinality(results_bm);
        roaring_bitmap_free(results_bm);
        auto end = std::chrono::high_resolution_clock::now();
        long long duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
...

    {
        ...
        for (size_t t = 0; t < BENCHMARK_NUM_THREADS; ++t) {
            query_threads.emplace_back([&, t]() {
                ...
                reader.get_objects_by_property_range_into_roaring("age", 30, 30, users_age_30_bm);
                roaring_bitmap_t* users_london_bm = roaring_bitmap_create();
                reader.get_objects_by_property_into_roaring("city", "London", users_london_bm);
        ...
        long long duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
        size_t items_processed = total_nodes_found.load() > 0 ? total_nodes_found.load() : 1;
        query_results.push_back({ "Filter (Age30 & London)", duration, static_cast<long long>(duration / items_processed), total_nodes_found.load(), "Total nodes: " + std::to_string(total_nodes_found.load()) });
    }
...

    {
        ...
        for (size_t t = 0; t < BENCHMARK_NUM_THREADS; ++t) {
            query_threads.emplace_back([&, t]() {
                GraphReader reader(db.get(), read_ctx);
                total_nodes_found += reader.count_objects_by_property("email", "__NULL_EMAIL__");
            });
        }
        for (auto& t : query_threads) t.join();
        auto end = std::chrono::high_resolution_clock::now();
        long long duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
        size_t items_processed = total_nodes_found.load() > 0 ? total_nodes_found.load() : 1;
        query_results.push_back({ "Nodes without Email", duration, static_cast<long long>(duration / items_processed), total_nodes_found.load(), "Total nodes: " + std::to_string(total_nodes_found.load()) });
    }

    {
        ...
        for (size_t t = 0; t < BENCHMARK_NUM_THREADS; ++t) {
            query_threads.emplace_back([&, t]() {
                ...
                for (size_t i = 0; i < NUM_QUERY_OPS / BENCHMARK_NUM_THREADS; ++i) {
                    uint32_t query_node_id = nodes[node_idx_dist(query_gen)].id;
                    local_followers += reader.get_outgoing_relationships(query_node_id, "FOLLOWS").size();
                }
        ...
        auto end = std::chrono::high_resolution_clock::now();
        long long duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
        query_results.push_back({ "1-Hop Out (FOLLOWS)", duration, static_cast<long long>(duration / NUM_QUERY_OPS), NUM_QUERY_OPS, "Total followers: " + std::to_string(total_followers_found.load()) });
    }
    
    {
        ...
        for (size_t t = 0; t < BENCHMARK_NUM_THREADS; ++t) {
            query_threads.emplace_back([&, t]() {
                ...
                for (size_t i = 0; i < NUM_QUERY_OPS / BENCHMARK_NUM_THREADS; ++i) {
                    uint32_t query_node_id = nodes[node_idx_dist(query_gen)].id;
                    local_incoming += reader.get_incoming_relationships(query_node_id, "FOLLOWS").size();
                }
        ...
        auto end = std::chrono::high_resolution_clock::now();
        long long duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
        query_results.push_back({ "1-Hop In (FOLLOWS)", duration, static_cast<long long>(duration / NUM_QUERY_OPS), NUM_QUERY_OPS, "Total incoming: " + std::to_string(total_incoming_followers.load()) });
    }

    {
        ...
        for (size_t t = 0; t < BENCHMARK_NUM_THREADS; ++t) {
            query_threads.emplace_back([&, t]() {
                ...
                reader.get_objects_by_property_into_roaring("city", "Paris", paris_users_bm);
                roaring_bitmap_t* london_follows = roaring_bitmap_create();
                reader.get_outgoing_relationships_for_many_into_roaring(london_users_bm, "FOLLOWS", london_follows);
        ...
        query_results.push_back({ "London->FOLLOWS->Paris", duration, static_cast<long long>(duration / items_processed), total_matches_found.load(), "Total matches: " + std::to_string(total_matches_found.load()) });
    }
    
    {
        ...
        for (size_t t = 0; t < BENCHMARK_NUM_THREADS; ++t) {
            query_threads.emplace_back([&, t]() {
                GraphReader reader(db.get(), read_ctx);
                total_users_count += reader.count_objects_by_property("type", "User");
            });
        }
        for (auto& t : query_threads) t.join();
        auto end = std::chrono::high_resolution_clock::now();
        ...
        query_results.push_back({ "Count Users", duration, static_cast<long long>(duration / items_processed), total_users_count.load(), "Total users: " + std::to_string(total_users_count.load()) });
    }

    {
        ...
        for (size_t t = 0; t < BENCHMARK_NUM_THREADS; ++t) {
            query_threads.emplace_back([&, t]() {
                GraphReader reader(db.get(), read_ctx);
                total_follows_count += reader.count_relationships_by_type("FOLLOWS");
            });
        }
        for (auto& t : query_threads) t.join();
        auto end = std::chrono::high_resolution_clock::now();
        ...
        query_results.push_back({ "Count FOLLOWS Rels", duration, static_cast<long long>(duration / items_processed), total_follows_count.load(), "Total FOLLOWS: " + std::to_string(total_follows_count.load()) });
    }

    {
        uint32_t test_node1_id = 101; 
        uint32_t test_node2_id = 201;
        
        std::atomic<size_t> total_common_neighbors_found = 0;
        auto start = std::chrono::high_resolution_clock::now();
        
        GraphReader reader(db.get(), read_ctx);
        auto pipeline = reader.get_common_neighbors(test_node1_id, test_node2_id, "FOLLOWS");
        uint32_t id;
        ...
        query_results.push_back({ "Common Neighbors (AND)", duration, static_cast<long long>(duration / items_processed), total_common_neighbors_found.load(), "Total common: " + std::to_string(total_common_neighbors_found.load()) });
    }

    {
        uint32_t test_node1_id = 102; 
        uint32_t test_node2_id = 103; 
        ...
        for (size_t t = 0; t < BENCHMARK_NUM_THREADS; ++t) {
            query_threads.emplace_back([&, t]() {
                GraphReader reader(db.get(), read_ctx);
                for(size_t i = 0; i < NUM_QUERY_OPS / BENCHMARK_NUM_THREADS; ++i) {
                    if (reader.has_relationship(test_node1_id, "FOLLOWS", test_node2_id) &&
                        reader.has_relationship(test_node2_id, "FOLLOWS", test_node1_id)) {
                        total_mutual_pairs++;
                    }
                }
            });
        ...
        auto end = std::chrono::high_resolution_clock::now();
        long long duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
        query_results.push_back({ "Mutual Rels (A<->B)", duration, static_cast<long long>(duration / NUM_QUERY_OPS), NUM_QUERY_OPS, "Total mutual pairs: " + std::to_string(total_mutual_pairs.load()) });
    }
    
    {
        uint32_t start_node = 101;
        uint32_t end_node = 501;
        
        auto start = std::chrono::high_resolution_clock::now();
        GraphReader reader(db.get(), read_ctx);
        std::vector<uint32_t> path = reader.find_shortest_path(start_node, end_node, "FOLLOWS");
        auto end = std::chrono::high_resolution_clock::now();
        long long duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
        size_t items_processed = path.size() > 0 ? path.size() : 1;
        query_results.push_back({ "Shortest Path (BFS)", duration, static_cast<long long>(duration / items_processed), path.size(), "Path len: " + std::to_string(path.size()) });
    }
    
    {
        auto start = std::chrono::high_resolution_clock::now();
        GraphReader reader(db.get(), read_ctx);
        uint64_t triangle_count = reader.count_triangles("FOLLOWS");
        auto end = std::chrono::high_resolution_clock::now();
        long long duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
        size_t items_processed = triangle_count > 0 ? triangle_count : 1;
        query_results.push_back({ "Triangle Counting", duration, static_cast<long long>(duration / items_processed), triangle_count, "Triangles: " + std::to_string(triangle_count) });
    }
