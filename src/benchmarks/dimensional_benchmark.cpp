...
void run_query_benchmarks() {
    ...
    for(int i=0; i < NUM_QUERIES; ++i) {
        ...
        std::vector<void*> results = tree.query_box(query_box);
    }
    ...
    for(int i=0; i < NUM_QUERIES; ++i) {
        std::vector<void*> results = tree.query_sphere(points[i].data(), 1500.0L);
    }
    ...
    for(int i=0; i < NUM_QUERIES; ++i) {
        std::vector<void*> results = tree.query_knn(points[i].data(), 10);
    }
