...
void run_spatial_workload(
    ...
    // 5. --- CORRECTNESS: KNN Query ---
    {
        ...
        for (int i = 0; i < tests_to_run; ++i) {
            ...
            // A. Get results from the StaxDB KNN implementation
            QueryStats knn_stats;
            std::vector<void*> knn_results_handles = tree.query_knn(center.data(), k, knn_stats);
            std::vector<std::vector<uint64_t>> knn_results_coords;
            for (void* handle : knn_results_handles) {
                const StaxRecord* rec = allocator.get_ptr<StaxRecord>(StaxTree16::get_offset(reinterpret_cast<uint64_t>(handle)));
                std::vector<uint64_t> coords(D);
                SpatialKeywords::get_coords_from_apk(rec->get_key(), coords.data(), D);
                knn_results_coords.push_back(coords);
            }

            // B. Get results from a brute-force linear scan (ground truth)
            // IMPORTANT: To ensure a fair comparison, the brute-force check must operate
            // on the same lossy, reconstructed coordinates that the KNN query sees.
            std::vector<std::pair<long double, std::vector<uint64_t>>> all_distances;
            for (const auto& p_original : inserted_points) {
                ...
                all_distances.push_back({SpatialKeywords::PointDistSq(center.data(), p_reconstructed.data(), D), p_reconstructed});
            }
            std::sort(all_distances.begin(), all_distances.end(), [](const auto& a, const auto& b){
                return a.first < b.first;
            });

            std::vector<std::vector<uint64_t>> brute_force_results_coords;
            for (int j = 0; j < std::min((int)all_distances.size(), k); ++j) {
                brute_force_results_coords.push_back(all_distances[j].second);
            }

            // C. Compare the results
            bool test_passed = true;
            if (knn_results_coords.size() != brute_force_results_coords.size()) {
                test_passed = false;
            } else {
                // Sort both by coordinates to handle potential distance ties differently
                auto coord_sorter = [D](const std::vector<uint64_t>& a, const std::vector<uint64_t>& b) {
                    for (uint32_t dim = 0; dim < D; ++dim) {
                        if (a[dim] != b[dim]) return a[dim] < b[dim];
                    }
                    return false;
                };
                std::sort(knn_results_coords.begin(), knn_results_coords.end(), coord_sorter);
                std::sort(brute_force_results_coords.begin(), brute_force_results_coords.end(), coord_sorter);

                for (size_t j = 0; j < knn_results_coords.size(); ++j) {
                    if (knn_results_coords[j] != brute_force_results_coords[j]) {
                        test_passed = false;
                        break;
                    }
                }
            ...
            if (!test_passed) {
                ...
                std::cerr << "  [FAIL] KNN Correctness test failed for query #" << i << std::endl;

                // --- DETAILED DEBUG LOGGING ---
                std::cerr << "    Query Point: (";
                for(uint32_t d=0; d<D; ++d) std::cerr << center[d] << (d==D-1 ? "" : ", ");
                std::cerr << ")" << std::endl;

                std::cerr << "    Expected (Brute Force):" << std::endl;
                for(size_t j=0; j<brute_force_results_coords.size(); ++j) {
                    std::cerr << "      - Point: (";
                    for(uint32_t d=0; d<D; ++d) std::cerr << brute_force_results_coords[j][d] << (d==D-1 ? "" : ", ");
                    std::cerr << "), DistSq: " << all_distances[j].first << std::endl;
                }

                std::cerr << "    Actual (StaxDB KNN):" << std::endl;
                for(const auto& coords : knn_results_coords) {
                     std::cerr << "      - Point: (";
                    for(uint32_t d=0; d<D; ++d) std::cerr << coords[d] << (d==D-1 ? "" : ", ");
                    std::cerr << "), DistSq: " << SpatialKeywords::PointDistSq(center.data(), coords.data(), D) << std::endl;
                }
                // --- END DEBUG LOGGING ---
         ...
         if (all_tests_passed) {
            std::cout << "--- ALL KNN CORRECTNESS TESTS PASSED ---" << std::endl;
        } else {
            std::cerr << "--- SOME KNN CORRECTNESS TESTS FAILED ---" << std::endl;
        }
    }

    // 6. --- BENCHMARK: KNN Query ---
    {
        QueryStats total_stats;
        long long total_duration_ns = 0;
        int k = static_cast<int>(target_records_in_query);
...
int main() {
    ...
    run_spatial_workload("16D Uniform", 16, 20000, 100, 0.01);
    run_spatial_workload("32D Uniform", 32, 20000, 100, 0.01);
    run_spatial_workload("64D Uniform", 64, 20000, 100, 0.01);
