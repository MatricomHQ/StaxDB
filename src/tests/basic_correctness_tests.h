...
void run_durability_test() {
    ...
    
    {
        ...
        users_col.insert(users_ctx, "user:1", "kris");
        ...
        orders_col.insert(orders_ctx, "order:101", "product_a");
...

    {
        ...
        if (!user_res || std::string_view(user_res->get_value_data(), user_res->value_len) != "kris") {
            std::cerr << "FAIL: Durability Phase 2 - user:1 value mismatch. Expected 'kris', got '" << (user_res ? std::string(user_res->get_value_data(), user_res->value_len) : "NOT_FOUND") << "'." << std::endl;
            test_passed = false;
        }
        ...
        if (!order_res || std::string_view(order_res->get_value_data(), order_res->value_len) != "product_a") {
            std::cerr << "FAIL: Durability Phase 2 - order:101 value mismatch. Expected 'product_a', got '" << (order_res ? std::string(order_res->get_value_data(), order_res->value_len) : "NOT_FOUND") << "'." << std::endl;
            test_passed = false;
        }
...

    {
        ...
        users_col_update.insert(users_update_ctx, "user:1", "kris_updated");
        users_col_update.commit(users_update_ctx);
...

    {
        ...
        if (!user_res || std::string_view(user_res->get_value_data(), user_res->value_len) != "kris_updated") {
            std::cerr << "FAIL: Durability Phase 4 - user:1 value mismatch. Expected 'kris_updated', got '" << (user_res ? std::string(user_res->get_value_data(), user_res->value_len) : "NOT_FOUND") << "'." << std::endl;
            test_passed = false;
        }
        ...
        if (!order_res || std::string_view(order_res->get_value_data(), order_res->value_len) != "product_a") {
            std::cerr << "FAIL: Durability Phase 4 - order:101 not found or value mismatch. Expected 'product_a', got '" << (order_res ? std::string(order_res->get_value_data(), order_res->value_len) : "NOT_FOUND") << "'." << std::endl;
            test_passed = false;
        }
...
void run_basic_correctness_test() {
    ...
    col1_ref.insert(ctx1, "users:kris", "kris_payload");
    col1_ref.commit(ctx1);

    Collection& col2_ref = db->get_collection_by_idx(col2_idx);
    TxnContext ctx2 = col2_ref.begin_transaction_context(0, false); 
    col2_ref.insert(ctx2, "orders:101", "order_101_payload");
    col2_ref.commit(ctx2);
    ...
    if (!res_user || std::string_view(res_user->get_value_data(), res_user->value_len) != "kris_payload") {
        std::cerr << "FAIL: Basic Correctness - users:kris value mismatch. Expected 'kris_payload', got '" << (res_user ? std::string(res_user->get_value_data(), res_user->value_len) : "NOT_FOUND") << "'." << std::endl;
        test_passed = false;
    }
    ...
    if (!res_order || std::string_view(res_order->get_value_data(), res_order->value_len) != "order_101_payload") {
        std::cerr << "FAIL: Basic Correctness - orders:101 value mismatch. Expected 'order_101_payload', got '" << (res_order ? std::string(res_order->get_value_data(), res_order->value_len) : "NOT_FOUND") << "'." << std::endl;
        test_passed = false;
    }
